...
module.exports = class ContactUsController {
    ...
    async contact(req, res, next) {
        try {
            let data = req.body;
            let rules = {first_name: 'required', email: 'required', message: 'required'};
            ...
            MailServiceObj.sendMail({to: ADMIN_EMAIL, html: admin_email_html, subject: 'New Query'}).then(async (result) => {
                if (result.sent == false) {
                    ...
                    FailedMailServiceObj.insertFailedMail({email: result.data.to, data: result.data, type: 'CONTACT_US_ADMIN'});
                }
            }).catch(async (ex) => {
                console.log(ex);
            });
            ...
            MailServiceObj.sendMail({to: data.email, html: customer_thankyou_html, subject: 'Thankyou For Writing To Us'}).then(async (result) => {
                if (result.sent == false) {
                    ...
                    FailedMailServiceObj.insertFailedMail({email: result.data.to, data: result.data, type: 'CONTACT_US_ENQUIRER'});
                }
            }).catch(async (ex) => {
                console.log(ex);
            });
